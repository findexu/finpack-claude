...
| `/quest-system` | *(manual only)* | RPG-themed expedition and epic management. Tracks quests (features/epics), expeditions (work loops), and realms (app targets) across expeditions using five persistent scrolls. Maintains `.ai-context/` (committed to git) for Copilot/Gemini compatibility — paste `quest.md` at chat start for instant context. Commands: `/set-bounty` (autonomous — post a goal; a party explores/plans/builds/self-tests/reviews and presents a near-done solution for sign-off), `/new-quest`, `/counsel-quest`, `/embark`, `/make-camp`, `/quest-log`, `/change-quest`, `/summon-witch-doctor` (scroll health diagnostics and repair). Run `/install-quest-system` once per project. |
